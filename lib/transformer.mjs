...
/**
 * TypeScript custom transformer that automatically adds file extensions to relative imports.
 * 
 * This transformer resolves relative imports without extensions by checking for:
 * - Original path + '.ts'
 * - Original path + '.js' 
 * - Original path + '/index.ts'
 * - Original path + '/index.js'
 * 
 * When a file is found, it updates the module specifier to include the extension,
 * converting .ts extensions to .js for runtime compatibility in ESM builds.
 * 
 * @returns {ts.TransformerFactory<ts.SourceFile>} TypeScript transformer factory
 */
